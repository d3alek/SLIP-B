...
#define PACKET_LENGTH_FIELD_SIZE         (128)  /**< Packet length field size in bits. */
...
void radio_configure()
{
    ...
    NRF_RADIO->TXPOWER = 0x04UL;
    unsigned char buf[32];
    sprintf((char*)buf, "Power: %x\n", NRF_RADIO->TXPOWER);
    simple_uart_putstring(buf);
    NRF_RADIO->FREQUENCY = 6UL;           // Frequency bin 7, 2407MHz
